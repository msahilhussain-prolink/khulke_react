...
const CometChatLocationMessageBubble = (props) => {
  ...
  const redirectGoogleMaps = () => {
    window.open(
      `http://maps.google.com?q=${props.message.data.customData.latitude},${props.message.data.customData.longitude}`
    );
  };
  ...
  if (props.message?.sender?.uid === loggedInUser?.uid) {
    message = (
      <React.Fragment>
        ...
        >
          <p
            css={messageTxtStyle(context)}
            className="message__txt"
            onClick={redirectGoogleMaps}
          >
            {Translator.translate("CUSTOM_MESSAGE_LOCATION", context.language)}
          </p>
        </div>
        <div
...
CometChatLocationMessageBubble.defaultProps = {
  theme: theme,
};

CometChatLocationMessageBubble.propTypes = {
  theme: PropTypes.object,
  message: PropTypes.object.isRequired,
};

export { CometChatLocationMessageBubble };
