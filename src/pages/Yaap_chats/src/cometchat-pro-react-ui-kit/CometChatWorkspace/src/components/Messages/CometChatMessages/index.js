...
class CometChatMessages extends React.PureComponent {
  ...
  constructor(props) {
    ...
    CometChat.getLoggedinUser()
      ...
      .catch((error) => {
        logger.error("()()", error);
        this.errorHandler("SOMETHING_WRONG");
      });
  ...
  handleDisappearChange = (val) => {
    this.setState({ ...this.state, disappearTime: val });
  };
  ...
  actionHandler = (action, messages, key, group, options) => {
    ...
    switch (action) {
      ...
      case enums.ACTIONS["MESSAGE_RECEIVED"]:
        {
          ...
          if (message.data.metadata.endTime) {
            message.data.metadata.time = parseInt(message.data.metadata.time);
            localStorage.setItem("disappear_time", message.data.metadata.time);
            localStorage.setItem(
              "disappear_end_time",
              message.data.metadata.endTime
            );
            localStorage.setItem(
              "disappear_foreign_last_id",
              message.data.metadata.foreignId
            );
            this.setState({
              ...this.state,
              disappearTime: message.data.metadata.time,
            });
          } else {
      ...
      case enums.ACTIONS["MESSAGE_SENT"]:
        ...
        break;
      case enums.ACTIONS["DISAPPEAR_CHANGE"]:
        this.setState({ ...this.state, disappear: messages });
        break;
      case enums.ACTIONS["ERROR_IN_SENDING_MESSAGE"]:
        this.messageSent(messages);
        break;
      ...
      case enums.ACTIONS["CLEAR_EDIT_PREVIEW"]:
        ...
        break;
      case enums.ACTIONS["PIN_MESSAGE"]:
        this.pinMessage(messages);
        break;
    ...
    let messageComponent = (
      <React.Fragment>
        ...
        >
          ...
          {this.getContext()?.type === CometChat.RECEIVER_TYPE.USER &&
          Object.keys(this.getContext().item).length &&
          this.getContext().item.blockedByMe ? null : (
            <>
              <CometChatMessageList
                ref={(el) => {
          ...
          this.getContext().type === CometChat.ACTION_TYPE.TYPE_USER ? (
            ...
            >
              <CometChatUserDetails
                handleDisappearChange={this.handleDisappearChange}
                id={
                  this.state.messageList[this.state.messageList.length - 1]?.id
                }
                lang={this.props.lang}
                disappear={this.state.disappearTime}
                actionGenerated={this.actionHandler}
              />
            </div>
          ) : this.getContext().type === CometChat.ACTION_TYPE.TYPE_GROUP ? (
