...
import { 
  ...
  CometChatDeleteMessageBubble, CometChatActionMessageBubble, CometChatLocationMessageBubble
} from "../"
...
class CometChatMessageList extends React.PureComponent {
	...
	componentDidMount() {
		CometChat.getLoggedinUser()
			...
			.catch(error => {logger.error("()()",error);this.props.actionGenerated(enums.ACTIONS["ERROR"], [], "SOMETHING_WRONG")});
			...
			this.MessageListManager.fetchPreviousMessages()
				.then(messageList => {
					...
					for(let i=0;i<messageList.length;i++){
						if(messageList[i].data?.metadata?.time) continue;
						newMessageList.push(messageList[i]);
					}

					let now = new Date();

					let end_time = new Date(localStorage.getItem("disappear_end_time"));
					if(now>end_time && parseInt(localStorage.getItem("disappear_time"))!==-1){
						let go = false;
						for(let i=0;i<newMessageList.length;i++){
							if(go===false) continue;
							if(parseInt(newMessageList[i].id)===parseInt(localStorage.getItem("disappear_last_id"))) go = true;
							try{
								CometChat.deleteMessage(newMessageList[i].id)
							} catch(E){}
						}
						end_time.setTime(now.getTime() + (parseInt(localStorage.getItem("disappear_time"))*60*60*1000));
						localStorage.setItem("disappear_end_time",end_time);
					}
	...
	onMessageReceived = message => {
		// if(message.data.metadata.disappear) return;
		//mark the message as delivered
		this.markMessageAsDelivered(message);
		...
		if (message.hasOwnProperty("parentMessageId") === false && this.props.hasOwnProperty("parentMessageId") === false) {
			...
			if (this.messagesEnd.scrollHeight - this.messagesEnd.scrollTop - this.messagesEnd.clientHeight <= 1) {
				if (this.messageCount > enums.CONSTANTS["MAX_MESSAGE_COUNT"]) {
					...
					logger.error(`enums.ACTIONS["MESSAGE_@"]`)
		...
		if (message.hasOwnProperty("deletedAt")) {
			...
			switch (message.type) {
				...
				case CometChat.MESSAGE_TYPE.AUDIO:
					...
					break;
				case "location":
					component = <CometChatLocationMessageBubble key={messageKey} message={message} actionGenerated={this.props.actionGenerated}></CometChatLocationMessageBubble>
					break;
				default:
					break;
			}
	...
	getComponent = (message, key) => {
		...
		switch (message.category) {
			...
			case CometChat.CATEGORY_CUSTOM:
				if (this.state.loggedInUser?.uid === message.sender?.uid || message.type==="location") {
	...
	render() {
		...
		this.props.messages.map((message, key) => {
			...
			if(parseInt(message.id)===parseInt(localStorage.getItem("disappear_foreign_last_id")) && done){
				if(localStorage.getItem("disappear_time")!==undefined && localStorage.getItem("disappear_time")!==null && parseInt(localStorage.getItem("disappear_time"))!==-1){
					done = false;
					let time = parseInt(localStorage.getItem("disappear_time"));
					if(time!==24){
						time = time / 24;
						time = `Disappearing Message is on for ${time} days`
					} else {
						time = `Disappearing Message is on for ${time} hours`
					}

					let disappearOn = (
						<div css={messageDateContainerStyle()} className="message__date">
							<span css={messageDateStyle(this.context)}>{time}</span>
						</div>
					);
