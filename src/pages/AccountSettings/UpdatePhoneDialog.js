...
const UpdatePhoneDialog = (props) => {
  ...
  const verifyOTP = async () => {
    ...
    let phone_number = otp_to.length === 10 ? "+91" + otp_to : otp_to;
    let otp =
      first.current.value +
      second.current.value +
      third.current.value +
      fourth.current.value;
    let url = `${REACT_APP_BASE_URL_FOR_USER}/verify-phone-otp/`;
    let data = JSON.stringify({ phone_number, otp });
    var config = {
      method: "post",
      url: url,
      headers: {
    ...
    axios(config)
      .then(function (response) {
        if (response.status === 253) {
          ...
          changeuserDetails();
          setOpen(true);
          moengageEvent("Update PhoneNumber", "User", {
          ...
          } else if (response.data["message"] === "OTP is not valid") {
            check_valid(false);
          }
          clearOTPfields();
        }
