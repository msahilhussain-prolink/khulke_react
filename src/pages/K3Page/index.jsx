...
import {getK3Posts} from "../.././redux/actions/k3Action";
import "./style.css";

const SnippetPage = () => {

  const dispatch = useDispatch();
  const { k3Posts } = useSelector((state) => state.k3Posts);
  ...
  const handleSearch = (e) => {
    if (e.key === "Enter") {
      ...
      moengageEvent("Search", "K3", { "Search Text": e.target.value });
  ...
  const handleScroll = (e) => {
    ...
    if (bottom) {
      dispatch(getK3Posts(k3Posts.length));
    }
  ...
  useEffect(() => {
    if (location?.state?.postId !== undefined) {
        dispatch(getK3Posts(0, location?.state?.postId));
    } else {
        dispatch(getK3Posts(0))
    }
  ...
  const isLastPost = currentIndex === k3Posts.length-1;
          ...
          <div className="headerSnippet">
            <div className="headerSnippetRightCol">
              <div style={{display: "flex"}}>
                ...
                />{" "}
                <h2 className="headText">{allWords.th.k3}</h2>
              </div>
            </div>
            <div className="headerSnippetLeftCol">
              ...
              >
                <AddCircleOutline />
                <span className="addBtnText">{allWords.K3.btnadd}</span>
              </button>
